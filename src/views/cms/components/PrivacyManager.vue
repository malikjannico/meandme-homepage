...
const form = ref({ title_en: '', title_de: '', data_privacy_text_en: '', data_privacy_text_de: '' })
const previewLanguage = ref('de')
const originalData = ref({})

function loadData() {
  if (contentStore.settings) {
    const section = contentStore.sectionTitles.find(s => s.section_key === 'privacy')
    form.value = {
      title_en: section?.title_en || '',
      title_de: section?.title_de || '',
      data_privacy_text_en: contentStore.settings.data_privacy_text_en || '',
      data_privacy_text_de: contentStore.settings.data_privacy_text_de || ''
    }
...
async function saveAll() {
  ...
  try {
    ...
    await supabase.from('site_settings').update({
      data_privacy_text_en: form.value.data_privacy_text_en,
      data_privacy_text_de: form.value.data_privacy_text_de
    }).eq('id', 1)
    ...
    <section class="edit-section md-card">
      ...
      <div v-if="!isEditing" class="legal-view-md">
        ...
        <div class="legal-homepage-preview">
          <div class="legal-content-preview ql-editor" v-html="previewLanguage === 'de' ? form.data_privacy_text_de : form.data_privacy_text_en"></div>
        </div>
      </div>
      <div v-else>
        <div class="editor-field-group">
          <label class="editor-label">Text (EN)</label>
          <RichTextEditor v-model="form.data_privacy_text_en" placeholder="Datenschutz auf Englisch..." />
        </div>
        <div class="editor-field-group">
          <label class="editor-label">Text (DE)</label>
          <RichTextEditor v-model="form.data_privacy_text_de" placeholder="Datenschutz auf Deutsch..." />
        </div>
